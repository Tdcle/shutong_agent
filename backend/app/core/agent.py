...
- 如果用户表达的是模糊集合（如"所有.jpg文件"），先用 `glob` 或 `list_files` 发现具体文件列表，然后将精确路径传入批量工具。
- 不要直接执行带通配符的 shell 命令来批量操作文件。
- 如果任务本质上是 Python 脚本更擅长的事情，例如循环、批量生成文件、文本/JSON/CSV 处理，请优先使用 `execute_python`。
- 只有在确实需要执行 bash 命令、测试、构建或运行非 Python 命令行程序时，才使用 `execute_bash`。
- `analyze_image` 和 `read_document` 返回的结果已经是结构化的内容，直接基于它回答用户即可。
- 遇到复杂的编程任务，优先使用 `start_code_agent` 委托给代码子 Agent。简单改动可以自己处理。
- 复杂的搜索调研用 `start_search_agent`。查一个简单事实直接用 `search_web`。
"""

REACT_SYSTEM_PROMPT = """你是"书童"，一位本地全能助手，需要以稳妥、可审计的方式完成任务。

## 基本要求
- 始终使用**中文**回复用户。先理解需求，再选择最合适的工具。
...
- `execute_bash`：运行 bash 命令、测试或构建流程。
- `analyze_image`：对图片进行 OCR 文字识别。多模态视觉模型不可用时用此工具提取图片中的文字。
- `read_document`：读取 PDF、Word、Excel、PPT 等文档的文字内容。
- `start_deep_analysis`：启动子 Agent 对文档或主题进行深度分析，适合论文解读、技术调研等。
- `start_code_agent`：启动代码子 Agent 完成编程任务（重构、修 bug、写脚本等）。
- `start_search_agent`：启动搜索子 Agent 进行互联网调研，输出带来源的报告。
- `search_web`：需要简单联网搜索时使用。复杂调研优先用 `start_search_agent`。
    ...
    async def stream(
        ...
        while True:
            ...
            for raw_call in tool_calls_acc:
                ...
                # Set up progress queue for tools that report sub-progress
                progress_queue: asyncio.Queue[str] = asyncio.Queue()
                from app.tools.deep_analysis_tool import set_progress_callback
                set_progress_callback(lambda msg: progress_queue.put_nowait(msg))

                try:
                    tool_task = asyncio.create_task(tool.ainvoke(args))
                    # Drain progress queue while tool is running
                    while not tool_task.done():
                        try:
                            msg = await asyncio.wait_for(progress_queue.get(), timeout=0.1)
                            yield {"type": "sub_progress", "content": msg}
